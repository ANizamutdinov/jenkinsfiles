...
def getContainerSas(def storage_account_name, def storage_container_name) {
    node ('master') {
        ...
        sh '''#!/bin/bash
                ...
                TOKEN=$(az storage container generate-sas --account-name ''' + storage_account_name + ''' --n ''' + storage_container_name + ''' --permissions lr --expiry $EXP --https-only  --as-user --auth-mode login)
                echo ${TOKEN} > ./sas
                az logout
                '''
